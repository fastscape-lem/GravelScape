...
# Note this is using: Multiflow sum_stack- Due to the nrec, this is different than the single flow sum_stack.
@numba.njit
...
#2) Optional code for a more detailed Multiflow Update of the source grain size (D0)
    #!! Note: this is for multi-flow routing. Due to the nrec, this is different than the single flow.
    #The grain size at the drainage source is used as the source throughout the drainage network 
    #(duplicate the grain size value on the surface at the source down the drainage network).
    #This creates a source grain size grid that will be used to calculate the grain size fining (later). 
    @numba.njit
    #Inputs: the drainage stack/recievers, number of recievers/weights, topography, bedrock grain size (D0), & surface grain size grid that is updated over time.
    def UpdateD0(stack,rec,D01Grid,surface,bed,D0,nrec,weights):
        nrec= np.where(nrec>0,nrec,0)
        #st= np.where(stack>0,stack,0)
        D02=np.asarray(D01Grid.copy()) #!!! COPY
        for i in stack: 
            for j in range(nrec[i]): 
                r=rec[i,j]
                r=np.where(r>0,r,0)
                if r==i: #This is another way to avoid negative/undefined recievers
                    continue
                #If bedrock is reached, set the source grain size to the bedrock D0.
                if (surface[i] == bed[i]): # if (surface[i] <= bed[i]): #smaller or equal to bedrock due to Fortran
                    D02[i]= D0
                #Determine the source grain size to be distributed down the network and used as the initial source D0 grain size.
                c=[D02[r],D02[i]]
                D02[r]= np.nanmax(c) #when two channels meet, the larger grain size will take priority. 
        D02=np.where(np.isnan(D02),D0,D02)
        return D02 #D02 is the source grain size grid.

    
#3) Integrate the Y star through the drainage network weighted by the drainage
